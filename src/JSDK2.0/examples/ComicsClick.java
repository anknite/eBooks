...
public class ComicsClick extends HttpServlet
{
		...
		try
		{
			...
			ResultSet rs=st.executeQuery("Select * from BookTab");
				...
				out.println("<title>History..</title>");
					...
					out.println("<form action=\"http://localhost:8080/servlet/SessionComics\">");
					...
					out.println("<font face=\"Mistral\" size=20 color=#208234><center>Comics</center>");
				    ...
				    while(rs.next())
				    {
				      ...
				      int s=rs.getInt(3);
				      if(sub.equals("Comics")==true)
				  	  {
						   out.println("<tr align=center>");
						   out.println("<td><input type=checkbox name=Comics value="+s+"></td>");
